...
logger.info(
    "Iniciando API Flask/Telegram | port=%s | public_url=%s | download_dir=%s",
    PORT,
    PUBLIC_URL or "(vacío)",
    DOWNLOAD_DIR,
)
            ...
            if not use_backup:
                ...
                async def backup_handler(event):
                    ...
                    try:
                        ...
                        logger.exception(f"Error en backup handler: {e}")
    ...
    try:
        ...
        async def azura_handler(event):
            try:
                ...
                logger.exception(f"Error en azura handler: {e}")

        # 🔹 ENVÍO ÚNICO del comando (solo una vez)
        logger.info(f"Enviando comando a Azura: {command}")
...
def clean_storage():
    ...
    que el disco de 512MB se llene. Se ejecuta en cada petición.
    ...
    try:
        ...
        logger.exception(f"[clean_storage] Error durante limpieza: {e}")
    finally:
        try:
...
def handle_unexpected_error(error):
    if isinstance(error, HTTPException):
        return error
    logger.exception("Error no controlado procesando la petición")
    return jsonify({"status": "error", "message": "Error interno del servidor"}), 500


@app.before_request
def before_each_request():
    """Ejecuta limpieza de almacenamiento en cada petición entrante."""
