...
def clean_storage():
    ...
    try:
        ...
        print(f"[clean_storage] Error durante limpieza: {e}")
...
def before_each_request():
    ...
    logger.info("→ %s %s | ip=%s", request.method, request.path, request.headers.get("Fly-Client-IP", request.remote_addr))
    clean_storage()
...
def after_each_request(response):
    ...
    logger.info("← %s %s | status=%s | %.2f ms", request.method, request.path, response.status_code, duration_ms)
    return response
...
def handle_unexpected_error(e):
    if isinstance(e, HTTPException):
        return e
    logger.exception("Error no controlado procesando %s %s", request.method, request.path)
...
if __name__ == "__main__":
    logger.info("Iniciando servidor Flask en 0.0.0.0:%s", PORT)
