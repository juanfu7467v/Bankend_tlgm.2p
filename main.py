...
# --- Logging robusto para Fly.io ---
logging.basicConfig(
    level=getattr(logging, LOG_LEVEL, logging.INFO),
    format="%(asctime)s %(levelname)s [%(name)s] %(message)s",
    force=True,
)
logger = logging.getLogger("bankend-tlgm-2p")

# --- Configuración Interna ---
DOWNLOAD_DIR = "downloads"
os.makedirs(DOWNLOAD_DIR, exist_ok=True)

# Ejecutar una limpieza inicial al arrancar la máquina
...
async def send_telegram_command(command: str, consulta_id: str = None, endpoint_path: str = None):
    ...
    try:
            ...
            if not use_backup:
                ...
                async def backup_handler(event):
                    ...
                    try:
                        ...
                        logger.exception("Error en backup handler")
        ...
        logger.exception("Error en send_telegram_command")
    ...
    try:
        ...
        async def azura_handler(event):
            try:
                ...
                logger.exception("Error en azura handler")

        # 🔹 ENVÍO ÚNICO del comando (solo una vez)
        logger.info("Enviando comando a Azura: %s", command)
        ...
        while (time.time() - start_time) < AZURA_TIMEOUT:
            if stop_collecting.is_set():
                logger.info("Respuesta recibida de Azura, deteniendo espera...")
                break
            
            # Si no hay respuesta después de 35 segundos, salir
            if (time.time() - start_time) >= AZURA_TIMEOUT:
                logger.warning("Timeout de 35 segundos alcanzado sin respuesta")
                break
        ...
        logger.exception("Error en send_azura_command")
        return {"status": "error", "message": str(e)}
    finally:
        if client:
            await client.disconnect()
...
MAX_STORAGE_MB = 400  # Umbral de limpieza (deja margen sobre los 512MB)
MAX_FILE_AGE_SECONDS = 3600  # Archivos más viejos que 1 hora se eliminan

def clean_storage():
    """
    Elimina archivos temporales del directorio de descargas para evitar
    que el disco de 512MB se llene. Se ejecuta de forma periódica.
    - Elimina archivos con más de 1 hora de antigüedad.
    - Si el uso total supera MAX_STORAGE_MB, elimina los más antiguos primero.
    """
    try:
        now = time.time()
        files_info = []
        ...
        for fname in os.listdir(DOWNLOAD_DIR):
            ...
            files_info.append((fpath, age, size))

        # Eliminar archivos viejos (> 1 hora)
        for fpath, age, size in files_info:
            if age > MAX_FILE_AGE_SECONDS:
                os.remove(fpath)

        # Recalcular uso total tras limpieza por edad
        total_bytes = sum(
        ...
        # Si sigue superando el umbral, borrar por orden de antigüedad
        ...
        logger.info("Limpieza de almacenamiento completada. Uso actual: %.2f MB", total_mb)
    except Exception as e:
        logger.exception("[clean_storage] Error durante limpieza")

def maybe_clean_storage():
...
def _client_ip():
    return request.headers.get("Fly-Client-IP") or request.headers.get("X-Forwarded-For") or request.remote_addr or "unknown"
...
def _install_signal_handlers():
    def _handle_shutdown_signal(signum, frame):
        ...
        logger.info("Recibida señal %s. Apagado controlado en progreso...", signal_name)

    try:
        signal.signal(signal.SIGTERM, _handle_shutdown_signal)
        signal.signal(signal.SIGINT, _handle_shutdown_signal)
    except Exception:
        logger.warning("No se pudieron instalar handlers de señal.")

_install_signal_handlers()

@app.before_request
def before_each_request():
    """Ejecuta limpieza y logging ligero en cada petición entrante."""
    request._start_time = time.time()
    logger.info("➡️ %s %s | ip=%s", request.method, request.path, _client_ip())

    # No bloquear el healthcheck ni la ruta raíz con limpieza pesada en cada llamada.
    if request.path not in ("/", "/health"):
        maybe_clean_storage()

@app.after_request
def after_each_request(response):
    started_at = getattr(request, "_start_time", time.time())
    duration_ms = (time.time() - started_at) * 1000
    logger.info("⬅️ %s %s | status=%s | %.2fms", request.method, request.path, response.status_code, duration_ms)
    return response

@app.route("/", methods=["GET"])
def root():
    """
    Ruta raíz requerida por Fly.io para confirmar que el servidor está activo.
    Evita errores 'Not Found' al despertar la máquina del estado suspendido.
    """
    return jsonify({"status": "ok", "message": "API activa"}), 200

@app.route("/health", methods=["GET"])
def health():
    """Health check usado por Fly.io para verificar que la app responde correctamente."""
...
def universal_handler(endpoint):
    ...
    if endpoint in ["status", "dni_nombres", "venezolanos_nombres"]:
...
def handle_unexpected_error(error):
    ...
    return jsonify({"status": "error", "message": "Error interno del servidor", "detail": str(error)}), 500

if __name__ == "__main__":
    logger.info("Iniciando API en Fly.io en el puerto %s", PORT)
