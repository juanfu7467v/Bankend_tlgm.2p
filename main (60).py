...
def get_command_and_param(path, request_args):
    ...
    mapping = {
        ...
        # --- 🆕 Nuevos comandos vehiculares y de identificación ---
        "placav":    f"/placav {p}",     # Tarjeta de Identificación Vehicular Física
        "citv":      f"/citv {p}",       # Consulta Revisión Técnica PDF
        "revisiones": f"/citv {p}",      # Alias de citv
        "rvt":       f"/citv {p}",       # Alias de citv
        "placab":    f"/placab {p}",     # Boleta Informativa Vehicular SUNARP PDF
        "licencia":  f"/licencia {p}",   # Licencia de Conducir Electrónica PDF
        "mtc":       f"/mtc {p}",        # Certificado Reporte MTC / Papeletas PDF
        "papeletas": f"/papeletas {p}",  # Papeletas SAT
        "soat":      f"/soat {p}",       # SOAT Digital PDF
        "placar":    f"/placar {p}",     # Reporte Registro Placa SUNARP PDF
    }
...
def universal_handler(endpoint):
    ...
    # --- 🆕 Aliases de citv: revisiones y rvt se resuelven dentro del mapping ---
