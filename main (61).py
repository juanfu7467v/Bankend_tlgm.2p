...
def universal_handler(endpoint):
    ...
    # --- 🆕 Rutas Azura específicas (placa) → @AzuraSearchServices_bot ---
    # placav, citv, revisiones, rvt, placab, papeletas, soat, placar
    AZURA_PLACA_ENDPOINTS = {
        "placav",     # Tarjeta de Identificación Vehicular Física
        "citv",       # Consulta Revisión Técnica PDF
        "revisiones", # Alias de citv
        "rvt",        # Alias de citv
        "placab",     # Boleta Informativa Vehicular - SUNARP
        "papeletas",  # Papeletas SAT
        "soat",       # SOAT Digital en PDF
        "placar",     # Reporte Registro Placa
    }

    # --- 🆕 Rutas Azura específicas (dni/carnet) → @AzuraSearchServices_bot ---
    # licencia, mtc
    AZURA_DNI_ENDPOINTS = {
        "licencia",   # Licencia de Conducir Electrónica PDF
        "mtc",        # Certificado Reporte MTC - Papeletas PDF
    }

    if endpoint in AZURA_PLACA_ENDPOINTS:
        placa = request.args.get("placa") or request.args.get("query")
        if not placa:
            return jsonify({"status": "error", "message": "Parámetro 'placa' faltante. Ejemplo: ?placa=ABC123"}), 400
        command = f"/{endpoint} {placa.strip().upper()}"
        result = run_azura_command(command, endpoint_path=f"/{endpoint}")
        return jsonify(result)

    if endpoint in AZURA_DNI_ENDPOINTS:
        dni = request.args.get("dni") or request.args.get("carnet_extranjeria") or request.args.get("query")
        if not dni:
            return jsonify({"status": "error", "message": "Parámetro 'dni' faltante. Ejemplo: ?dni=45454545"}), 400
        command = f"/{endpoint} {dni.strip()}"
        result = run_azura_command(command, endpoint_path=f"/{endpoint}")
        return jsonify(result)
